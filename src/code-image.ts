...
export class CodeImagePanel {
  ...
  private _startListeningForSelectionChanges() {
    ...
    // 设置新的监听器
    this._selectionChangeDisposable =
  ...
  public static async createOrShow(
    ...
    const panel = vscode.window.createWebviewPanel(
      ...
      vscode.ViewColumn.Beside,
      {
        ...
        localResourceRoots: [vscode.Uri.joinPath(extensionContext.extensionUri)]
      }
    );
  ...
  private async _getHtmlContent(
    ...
    const baseUrl = this._panel.webview
      .asWebviewUri(
        vscode.Uri.file(path.resolve(this._extensionContext.extensionPath))
      )
      .toString();
    ...
    htmlTemplate = htmlTemplate
      ...
      .replace(
        /%ENABLE_PEN_TEXT%/g,
        vscode.l10n.t('Enable Pen, DoubleClick to clear')
      )
      .replace(
        /%DISABLE_PEN_TEXT%/g,
        vscode.l10n.t('Disable Pen, DoubleClick to clear')
      )
